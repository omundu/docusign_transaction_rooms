module DocusignTransactionRooms
  class DocumentResource < ResourceKit::Resource
    ...
    resources do
      ...
      action :destory do
        verb :delete
        path "#{DOCUSIGN_TRANSACTION_ROOMS_API_PATH}/documents/:id"
        handler(204) { |_| true }
      end
      ...
      action :members do
        verb :get
        path "#{DOCUSIGN_TRANSACTION_ROOMS_API_PATH}/documents/:id/access"
      ...
      action :revoke_access do
        verb :delete
        path "#{DOCUSIGN_TRANSACTION_ROOMS_API_PATH}/documents/:id/access/:user_id"
        handler(204) { |_| true }
      end
      ...
      action :details do
        verb :get
        path "#{DOCUSIGN_TRANSACTION_ROOMS_API_PATH}/documents/:id/details"
        handler(200) { |response| DocumentMapping.extract_single(response.body, :read) }
      end
    end
